...
const trails = [
 ...
 },
 {
   ...
   stageDistances: "8-19 km", services: "Satisfactory",
   stages: 27,
...
let filterByRegion = (region) => {
	let results = trails.filter(trail => {
...
let capitalize = (str) => {
	let capitalized = str.replace(/[^a-z]/g, " ").replace(/\b\w/g, char => {
  	return char.toUpperCase();
  });
  return capitalized;
};

let showProvinces = (region) => {
	let display = provinces[region];
  return display;
};

let showAllProvinces = () => {
  let two = provinces["svealand"].concat(provinces["northern-norrland"]);
  let three = two.concat(provinces["southern-norrland"]);
  let four = three.concat(provinces["northern-gotaland"]);
  let all = four.concat(provinces["southern-gotaland"]);
  return all;
};
